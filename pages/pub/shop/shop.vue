					...
					<view class="swiper-item evaluate">
						...
						<view class="evaluate-grade">
							<view class="garde-left">
								<view class="garde-num">{{evaluateScore}}</view>
							...
							<view class="garde-right">
								<view class="garde-right-row">
									...
									<view class="star-box">
										<text v-for="item in starNum(serverScore) " class="garde-row-item" style="font-size: 24upx;">⭐</text>
									</view>								
									<text class="garde-row-item" style="font-size: 28upx;color: #f60;">{{serverScore}}</text>
								</view>
								<view class="garde-right-row">
									<text class="garde-row-item garde-row-title">菜品评价</text>
									<view class="star-box">
									<text v-for="item in starNum(foodScore) " class="garde-row-item" style="font-size: 24upx;">⭐</text>
									</view>								
									<text class="garde-row-item" style="font-size: 28upx;color: #f60;">{{foodScore}}</text>
								</view>
								<view class="garde-right-row">
									<text class="garde-row-item garde-row-title">送达时间</text>				
									<text class="garde-row-item" style="font-size: 24upx;color: #666;">{{deliverTime}}分钟</text>
								</view>
							</view>
						</view>
						<!-- 评价内容模块-->
							...
							<view class="evaluate-article">
								...
								<view class="evaluate-article-item" v-for="item in evaluateContent">
									...
									<view class="evaluate-image-box">
										<image :src="'https://fuss10.elemecdn.com/'+item.avatar" mode="aspectFill"></image>
									</view>
									<!-- 内容包 -->
									<view class="content-box">
										...
										<view class="commodity-image-row">
											<image v-for="commodity in item.item_ratings" :src="'https://fuss10.elemecdn.com/'+commodity.image_hash" mode="aspectFill"></image>
										</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			//请求评价分数
			$comment_scores(this.shopId).then((el)=>{
				console.log(el)
				this.evaluateScore =  el.data.overall_score.toFixed(1)-0
				this.deliverTime = el.data.deliver_time
				this.serverScore = el.data.service_score.toFixed(1)-0
				this.foodScore = el.data.food_score.toFixed(1)-0
			})
		},
		onReady() {
